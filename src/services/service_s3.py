...
from ..utils.constants import ALLOWED_EXTENSIONS, DEFAULT_NAME, FORMAT_DOCUMENT, TEMP_DOCS
...
class ServiceS3:
        ...
        try:
            for file_path in self.get_path(file_json):
                if any(file_path.endswith(ext) for ext in ALLOWED_EXTENSIONS):
                    ...
                    try:
                        s3_response = self.s3.cnx.get_object(
                            Bucket=AWS_S3_BUCKET_NAME, Key=object_key
                        )
    ...
    def split_large_files_pdf_s3(self):
        try:
            s3_response = self.s3.cnx.list_objects_v2(
                Bucket=AWS_S3_BUCKET_NAME, Prefix=TEMP_DOCS
            )
                        ...
                        for i, start_page in enumerate([0, mid_point]):
                            ...
                            for page_num in range(start_page, start_page + mid_point):
                                if page_num < num_pages:
                                    pdf_writer.add_page(pdf.get_page(page_num))
                            ...
                            self.s3.cnx.put_object(
                                Bucket=AWS_S3_BUCKET_NAME,
                                Key=new_object_key,
                                Body=output_pdf.getvalue(),
                            )
                    else:
                        self.s3.cnx.put_object(
                            Bucket=AWS_S3_BUCKET_NAME, Key=object_key, Body=content
                        )
            return True
        except Exception as e:
            raise e
